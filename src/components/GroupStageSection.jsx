...
function MatchRow({ match, pick, result, onPickChange, confirmed }) {
  ...
  function getMatchClass() {
    if (!result || result.home_goals === null || result.home_goals === undefined) return '';
    ...
    <div className={`match-row ${getMatchClass()} ${isHost ? 'match-double' : ''}`}>
      ...
      <div className="match-body">
        ...
        {result && result.home_goals !== null && result.home_goals !== undefined && (
          <span className="real-result">Real: {result.home_goals}-{result.away_goals}</span>
        )}
      </div>
    </div>
  );
...
function GroupTable({ teams, groupId, allMatches, picks, results }) {
  ...
  return (
    <div className="group-table">
      <table>
        <thead>
          <tr><th>#</th><th>Selección</th><th>PJ</th><th>G</th><th>E</th><th>P</th><th>GF</th><th>GC</th><th>GD</th><th>Pts</th></tr>
        </thead>
        <tbody>
          {sorted.map((row, i) => (
            <tr key={row.team} className={i < 2 ? 'qualifies-direct' : ''}>
              <td className="pos-num">{i + 1}</td>
              <td className="team-cell"><span className="team-flag">{FLAGS[row.team] || ''}</span>{row.team}{HOST_TEAMS.includes(row.team) && <span className="host-star">★</span>}</td>
      ...
      <p className="table-note">Clasifican directamente los 2 primeros. Los mejores 8 terceros también avanzan.</p>
    </div>
  );
}
    ...
    <div className="group-stage-section">
      <div className="group-tabs">
        {groupIds.map(gId => {
          ...
          return (
            <button key={gId} className={`group-tab ${activeGroup === gId ? 'active' : ''} ${done ? 'done' : ''}`} onClick={() => setActiveGroup(gId)}>
              {gId}<span className="group-tab-progress">{prog.filled}/{prog.total}</span>
            </button>
          );
        })}
      </div>
          ...
          <div className="group-teams-preview">
            {GROUPS[activeGroup].teams.map(t => (
              <span key={t} className="team-pill">
                {FLAGS[t]} {t}
                {HOST_TEAMS.includes(t) && <span className="host-badge">Anfitrión ×2</span>}
              </span>
            ))}
          </div>
        </div>
